...
class StudentListView < FXMainWindow
  ...
  def setup_crud_segment(parent)
    ...
    add_button.setBackColor(FXRGB(255, 105, 180)) 
    ...
    delete_button.setBackColor(FXRGB(255, 105, 180)) 
    ...
    edit_button.setBackColor(FXRGB(255, 105, 180)) 
    ...
    refresh_button.setBackColor(FXRGB(255, 105, 180)) 
  ...
  def add_filter_row(parent, label)
    FXLabel.new(parent, label, opts: LABEL_NORMAL).setTextColor(FXRGB(255, 105, 180)) # Ярко-розовый
    ...
    cbx.setBackColor(FXRGB(255, 192, 203)) # Розовый фон для ComboBox
    cbx.setTextColor(FXRGB(0, 0, 0)) # Черный текст для ComboBox
  
    search_tbx = FXTextField.new(parent, 25)
    search_tbx.setBackColor(FXRGB(255, 192, 203)) # Розовый фон для текстового поля
